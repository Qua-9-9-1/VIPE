...
void Canva::cursor_square(int x1, int y1, int size, bool smooth) {
    ...
    if (_prev_x != -1 && _prev_y != -1) {
        ...
        for (int i = 0; i <= steps; ++i) {
            ...
            cv::Point top_left((xi - size / 2) - (is_selection_active ? _selected_region.x : 0),
                               (yi - size / 2) - (is_selection_active ? _selected_region.y : 0));
            cv::Point bottom_right((xi + size / 2) - (is_selection_active ? _selected_region.x : 0),
                                   (yi + size / 2) -
                                       (is_selection_active ? _selected_region.y : 0));
...
void Canva::flood_fill(int x, int y, double tolerance) {
    ...
    if (is_selection_active) {
        ...
        int     sel_x          = x - _selected_region.x;
        int     sel_y          = y - _selected_region.y;
    ...
    if (is_selection_active) {
        cv::Mat  selection_mask = _selection.get_mask();
        cv::Rect selection_rect(_selected_region.x, _selected_region.y, selection_mask.cols,
                                selection_mask.rows);
